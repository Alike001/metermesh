...
description: >
  ...
  Geist Variable in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  # — reading ramp —
  body:    { fontFamily: "Geist Variable", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Geist Variable", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Geist Variable", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 560, tracking: "0.14em", upper: true }
  # — display / hero ramp (Geist Variable, lowercase, negative tracking) —
  h3:      { fontFamily: "Geist Variable", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Geist Variable", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Geist Variable", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Geist Variable", cqw: 5.5, weight: 700, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Geist Variable", cqw: 7.5, weight: 700, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Geist Variable", cqw: 7.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Geist Variable", cqw: 10.0, weight: 700, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Geist Variable", cqw: 10.5, weight: 700, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Geist Variable", cqw: 13.0, weight: 700, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Geist Variable** (display) / **Geist Mono Variable** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 560, 590, 600, 620, 660, 700}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
text and becomes graphic primitive.** Geist Variable `display` at 13cqw puts a single lowercase word
...
**Geist Variable** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Geist Variable 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Geist Variable body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Geist Variable `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Geist Variable display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Geist Variable `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Geist Variable lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Geist Variable `display`/`h1`
...
row of three top-border-only stat-cards — big Geist Variable-900 numeral in `{colors.fire-orange}`, Geist Variable
...
**Ground** ink-black. **Composes** fadelist (3 stacked Geist Variable-900 words at 1.0/0.5/0.22), fadelist-title.
...
Geist Variable `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Geist Variable display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Geist Variable display; never add a second accent color.
...
- No serif companion; chrome is never Geist Variable.
...
- **Type** — Geist Variable lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Geist Variable + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
